 ...
 * 里面的值都是0-1之间
 */
public class GPUImageHueColorBlendFilter extends GPUImageFilter {
    public static final String HUE_BLEND_FRAGMENT_SHADER = "varying highp vec2 textureCoordinate;\n" +
            ...
            "     \n" +
            "     gl_FragColor = vec4(base.rgb * (1.0 - overlay.a) + setlum(setsat(overlay.rgb, sat(base.rgb)), lum(base.rgb)) * overlay.a, base.a);\n" +
            " }";

    private float[] mOverlay;
    private int mOverlayLocation;

    public GPUImageHueColorBlendFilter() {
        this(new float[]{1.0f, 0f, 0f, 1f});
    }

    public GPUImageHueColorBlendFilter(float[] rgba) {
        super(NO_FILTER_VERTEX_SHADER, HUE_BLEND_FRAGMENT_SHADER);
        this.mOverlay = rgba;
    }
