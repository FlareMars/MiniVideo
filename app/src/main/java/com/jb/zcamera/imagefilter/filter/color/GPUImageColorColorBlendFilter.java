 ...
 * 里面的值都是0-1之间
 */
public class GPUImageColorColorBlendFilter extends GPUImageFilter {
    public static final String COLOR_BLEND_FRAGMENT_SHADER = "varying highp vec2 textureCoordinate;\n" +
            ...
            "     \n" +
            "     gl_FragColor = vec4(base.rgb * (1.0 - overlay.a) + setlum(overlay.rgb, lum(base.rgb)) * overlay.a, base.a);\n" +
            " }";

    private float[] mOverlay;
    private int mOverlayLocation;

    public GPUImageColorColorBlendFilter() {
        this(new float[]{1.0f, 0f, 0f, 1f});
    }

    public GPUImageColorColorBlendFilter(float[] rgba) {
        super(NO_FILTER_VERTEX_SHADER, COLOR_BLEND_FRAGMENT_SHADER);
        this.mOverlay = rgba;
    }
