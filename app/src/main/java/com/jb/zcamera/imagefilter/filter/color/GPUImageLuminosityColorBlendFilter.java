 ...
 * 里面的值都是0-1之间
 */
public class GPUImageLuminosityColorBlendFilter extends GPUImageFilter {
    public static final String LUMINOSITY_BLEND_FRAGMENT_SHADER = "varying highp vec2 textureCoordinate;\n" +
            ...
            "     \n" +
            "     gl_FragColor = vec4(base.rgb * (1.0 - overlay.a) + setlum(base.rgb, lum(overlay.rgb)) * overlay.a, base.a);\n" +
            " }";

    private float[] mOverlay;
    private int mOverlayLocation;

    public GPUImageLuminosityColorBlendFilter() {
        this(new float[]{1.0f, 0f, 0f, 1f});
    }

    public GPUImageLuminosityColorBlendFilter(float[] rgba) {
        super(NO_FILTER_VERTEX_SHADER, LUMINOSITY_BLEND_FRAGMENT_SHADER);
        this.mOverlay = rgba;
    }
