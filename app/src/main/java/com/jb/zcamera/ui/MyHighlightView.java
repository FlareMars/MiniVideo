...
public class MyHighlightView implements EditableDrawable.OnSizeChange {
    ...
    private static final String LOG_TAG = "drawable-view";
    ...
    public interface OnDeleteClickListener {
        void onDeleteClick(MyHighlightView highlightView);
    }
    ...
    public void setScaleable(boolean scaleable) {
        ...
        if (scaleable) {
            mAnchorRotate = CameraApp.getApplication().getResources().getDrawable(R.drawable.aviary_resize_knob);
        } else {
            mAnchorRotate = null;
        }
    }
    ...
    public void setDeleteable(boolean deleteable) {
        if (deleteable) {
            mAnchorDelete = CameraApp.getApplication().getResources().getDrawable(R.drawable.aviary_delete_knob);
        } else {
            mAnchorDelete = null;
        }
    }
    ...
    public void draw(final Canvas canvas) {
        ...
        if (is_selected || is_focused) {
            ...
            if (mShowAnchors) {
                ...
                outlinePaint.setStrokeWidth(ImageHelper.dpToPx(CameraApp.getApplication().getResources(), 1));
    ...
    public int getHit(float x, float y) {
        ...
        if (mScaleEnabled) {
            ...
            if ((Math.abs(rect.left - x) < HIT_TOLERANCE) && verticalCheck
                    && ZCameraUtil.checkBits(mResizeEdgeMode, GROW_LEFT_EDGE)) {
                Log.d(LOG_TAG, "left");
                retval |= GROW_LEFT_EDGE;
            }
            if ((Math.abs(rect.right - x) < HIT_TOLERANCE) && verticalCheck
                    && ZCameraUtil.checkBits(mResizeEdgeMode, GROW_RIGHT_EDGE)) {
                Log.d(LOG_TAG, "right");
                retval |= GROW_RIGHT_EDGE;
            }
            if ((Math.abs(rect.top - y) < HIT_TOLERANCE) && horizCheck
                    && ZCameraUtil.checkBits(mResizeEdgeMode, GROW_TOP_EDGE)) {
                Log.d(LOG_TAG, "top");
                retval |= GROW_TOP_EDGE;
            }
            if ((Math.abs(rect.bottom - y) < HIT_TOLERANCE) && horizCheck
                    && ZCameraUtil.checkBits(mResizeEdgeMode, GROW_BOTTOM_EDGE)) {
        ...
        if (mAnchorDelete != null) {
            if ((Math.abs(rect.left - x) < HIT_TOLERANCE)
                ...
                if (mDeleteClickListener != null) {
                    mDeleteClickListener.onDeleteClick(this);
                }
            }
        }
    }
    ...
    public void onMouseMove(int edge, MotionEvent event2, float dx, float dy) {
        ...
        if (edge == MOVE) {
            ...
            boolean is_left = ZCameraUtil.checkBits(edge, GROW_LEFT_EDGE);
            boolean is_top = ZCameraUtil.checkBits(edge, GROW_TOP_EDGE);
    ...
    void rotateBy(final float dx, final float dy, float diffx, float diffy) {
        ...
        if (mScaleEnabled) {
            ...
            final double distance2 = Point2D.distance(pt1, pt3);
