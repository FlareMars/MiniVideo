...
public class DrawableOverlayImageView extends ImageViewTouch {
    ...
    public DrawableOverlayImageView(Context context, AttributeSet attrs) {
        super(context, attrs);
        //setScrollEnabled(false);
    }

    public DrawableOverlayImageView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public boolean addBackgroundHighlightView(MyHighlightView hv) {
        for (int i = 0; i < mOverlayViews.size(); i++) {
            if (mOverlayViews.get(i).equals(hv)) {
                return false;
            }
        ...
        if (mOverlayViews.size() > 0) {
            if (!mOverlayViews.get(0).isSelectable()) {
                mOverlayViews.remove(0);
            }
        }
        mOverlayViews.add(0, hv);
        postInvalidate();
    ...
    public void setSelectedHighlightView(MyHighlightView newView) {
        if (newView != null && !newView.isSelectable()) {
            return;
        }
