...
/**
 * Created by chenfangyi on 16-6-3.
 * 这个是图片的日期标记的公共类
 */
public class DateMaskUtil {

    //存在标记的时候多一个后缀
    private static final String END_PENDING = "_ZDateMark";

    /**
     * 用于存储当前标记是否开启
     */
    public static final String PREF_DATA_MARK_OPEN = "pref_data_mask_open";

    //用于存储当前的标记的格式
    public static final String PREF_CUR_FORMAT_STRING = "pref_cur_format_string";
    ...
    public static void saveCurFormat(String format){
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(CameraApp.getApplication());
        sp.edit().putString(PREF_CUR_FORMAT_STRING, format).apply();
    }
    ...
    public static void saveCurFormat(int index){
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(CameraApp.getApplication());
        sp.edit().putString(PREF_CUR_FORMAT_STRING, DEFAULT_FORMAT[index]).apply();
    }
    ...
    public static String getCurFormat(){
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(CameraApp.getApplication());
        return sp.getString(PREF_CUR_FORMAT_STRING, DEFAULT_FORMAT[0]);
    }
    ...
    public static void setDataMarkOpen(boolean open){
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(CameraApp.getApplication());
        sp.edit().putBoolean(PREF_DATA_MARK_OPEN, open).apply();
    }
    ...
    public static boolean getDataMarkOpen(){
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(CameraApp.getApplication());
        return sp.getBoolean(PREF_DATA_MARK_OPEN, false);
    }
